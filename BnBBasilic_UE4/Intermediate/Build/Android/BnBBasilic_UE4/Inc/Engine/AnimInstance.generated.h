...
#define Engine_Source_Runtime_Engine_Classes_Animation_AnimInstance_h_354_RPC_WRAPPERS \
 ...
 \
	DECLARE_FUNCTION(execGetLayerSubInstanceByGroup) \
	{ \
		P_GET_PROPERTY(UNameProperty,Z_Param_InGroup); \
		P_FINISH; \
		P_NATIVE_BEGIN; \
		*(UAnimInstance**)Z_Param__Result=P_THIS->GetLayerSubInstanceByGroup(Z_Param_InGroup); \
		P_NATIVE_END; \
	} \
 \
	DECLARE_FUNCTION(execSetLayerOverlay) \
	{ \
		P_GET_OBJECT(UClass,Z_Param_InClass); \
		P_FINISH; \
		P_NATIVE_BEGIN; \
		P_THIS->SetLayerOverlay(Z_Param_InClass); \
		P_NATIVE_END; \
	} \
 \
	DECLARE_FUNCTION(execSetSubInstanceClassByTag) \
	{ \
		P_GET_PROPERTY(UNameProperty,Z_Param_InTag); \
		P_GET_OBJECT(UClass,Z_Param_InClass); \
		P_FINISH; \
		P_NATIVE_BEGIN; \
		P_THIS->SetSubInstanceClassByTag(Z_Param_InTag,Z_Param_InClass); \
		P_NATIVE_END; \
	} \
 \
	DECLARE_FUNCTION(execGetSubInstancesByTag) \
	{ \
		P_GET_PROPERTY(UNameProperty,Z_Param_InTag); \
		P_GET_TARRAY_REF(UAnimInstance*,Z_Param_Out_OutSubInstances); \
		P_FINISH; \
		P_NATIVE_BEGIN; \
		P_THIS->GetSubInstancesByTag(Z_Param_InTag,Z_Param_Out_OutSubInstances); \
		P_NATIVE_END; \
	} \
 \
	DECLARE_FUNCTION(execGetSubInstanceByTag) \
	{ \
		P_GET_PROPERTY(UNameProperty,Z_Param_InTag); \
		P_FINISH; \
		P_NATIVE_BEGIN; \
		*(UAnimInstance**)Z_Param__Result=P_THIS->GetSubInstanceByTag(Z_Param_InTag); \
		P_NATIVE_END; \
	} \
 \
